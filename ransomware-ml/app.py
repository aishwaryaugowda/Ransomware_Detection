...
# ============================================================================
# COMPLETE RANSOMWARE DETECTION SYSTEM
# ============================================================================
...
print("📥 Downloading CIC-Evasive-PDFMal2022 Dataset...")
print("Note: You need to upload the dataset manually or download from Kaggle")

# Load the dataset from local file
dataset_file = "C:\\Users\\aishw\\OneDrive\\Desktop\\ransomware-ml\\data\\PDFMalware2022.parquet"
...
if y.dtype == 'object' or pd.api.types.is_categorical_dtype(y):
    ...
    print(f"\n🏷️  Classes: 0=Benign, 1=Malicious")
...
print("\n" + "="*80)
print("🔍 INSPECTING DATASET FEATURES FOR PROPER MAPPING")
print("="*80)

print(f"\n📋 All {len(feature_names)} features in the dataset:")
...
print("\n" + "="*80)
print("🔧 IMPROVED FEATURE EXTRACTION FUNCTIONS")
print("="*80)
...
class ImprovedFeatureExtractor:
    ...
    def __init__(self, feature_names, feature_stats):
        ...
        print(f"📋 Initialized with {self.num_features} features")
    ...
    def extract_pdf_features(self, file_content):
        ...
        try:
            ...
            if non_zero:
                print(f"✅ PDF: Extracted {len([v for v in features.values() if v != 0])} non-zero features")
    ...
    def extract_docx_features(self, file_content):
        ...
        try:
            ...
            print(f"✅ DOCX: Extracted {len([v for v in features.values() if v != 0])} non-zero features")
    ...
    def extract_json_features(self, file_content):
        ...
        try:
            ...
            print(f"✅ JSON: Extracted {len([v for v in features.values() if v != 0])} non-zero features")
...
print("✅ Improved feature extraction ready!")
...
def predict_file_malware(file):
    ...
    try:
        ...
        print(f"🎯 Ensemble: Benign={ensemble_proba[0]:.4f}, Malicious={ensemble_proba[1]:.4f}")
        print(f"📊 Final: {'MALICIOUS' if prediction_idx == 1 else 'BENIGN'} ({confidence*100:.2f}%)")
        print(f"{'='*60}\n")

        # Determine prediction label
        prediction_label = "🦠 MALICIOUS" if prediction_idx == 1 else "✅ BENIGN"
        ...
        for idx in top_features_idx:
            ...
            important_features.append(f"  • {feat_name}: {feat_value:.2f} (importance: {feat_importance:.4f})")
...
• Random Forest: {"Malicious" if np.argmax(rf_proba) == 1 else "Benign"} ({rf_proba[np.argmax(rf_proba)] * 100:.2f}%)
• XGBoost: {"Malicious" if np.argmax(xgb_proba) == 1 else "Benign"} ({xgb_proba[np.argmax(xgb_proba)] * 100:.2f}%)
• Neural Network: {"Malicious" if np.argmax(nn_proba) == 1 else "Benign"} ({nn_proba[np.argmax(nn_proba)] * 100:.2f}%)
...
• Always exercise caution with files from unknown sources
• Keep your antivirus software updated
• Scan files with multiple security tools for maximum protection
"""
...
with gr.Blocks(title="🛡️ Malware Detection System", css=custom_css, theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            scan_button = gr.Button("🔍 Scan for Malware", variant="primary", size="lg")
        ...
        with gr.Column(scale=2):
            ...
            with gr.Row():
                prediction_output = gr.Textbox(label="🎯 Prediction", interactive=False, lines=1)
                confidence_output = gr.Textbox(label="📈 Confidence", interactive=False, lines=1)
                threat_output = gr.Textbox(label="⚠️ Threat Level", interactive=False, lines=1)

            gr.Markdown("### 🤖 Individual Model Results")

            with gr.Row():
                rf_conf = gr.Textbox(label="🌲 Random Forest", interactive=False, lines=1)
                xgb_conf = gr.Textbox(label="⚡ XGBoost", interactive=False, lines=1)
                nn_conf = gr.Textbox(label="🧠 Neural Network", interactive=False, lines=1)

            details_output = gr.Markdown(label="📄 Detailed Report")
...
print("\n" + "="*80)
print("🚀 LAUNCHING RANSOMWARE DETECTION SYSTEM")
print("="*80)
...
print("🎉 Upload files is detected")
